...
export default function OrderDashboard() {
  ...
  const removeFromCart = (itemId) => {
    setCart(prev => {
      const existing = prev.find(p => p._id === itemId);
      if (existing.quantity === 1) {
        const newCart = prev.filter(p => p._id !== itemId);
        if (newCart.length === 0) setIsCartOpen(false); // Add auto-close on empty
        return newCart;
      }
      return prev.map(p => p._id === itemId ? { ...p, quantity: p.quantity - 1 } : p);
    });
  };
  ...
  const submitOrder = async () => {
    ...
    try {
      ...
      await axios.post('http://localhost:5000/api/orders', { tableNumber: Number(tableNumber), items });
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 h-auto lg:h-[calc(100vh-8rem)] pb-24 lg:pb-0 relative">
            ...
            {categories.map(cat => (
              <button
                ...
                className={`snap-start px-5 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all duration-300 ${
                  activeCategory === cat ? 'bg-gradient-to-r from-brand-orange-500 to-amber-500 text-white shadow-[0_4px_15px_rgba(249,115,22,0.4)] transform scale-105' : 'bg-white/10 text-gray-300 hover:bg-white/20 border border-white/5 hover:text-white'
                }`}
              >
                {cat}
              </button>
        ...
        <div className="flex-1 overflow-y-auto p-5 md:p-6 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 h-max">
            {filteredMenu.map(item => {
              ...
              return (
              <div key={item._id} className="bg-white/10 backdrop-blur-md rounded-2xl border border-white/10 shadow-lg hover:bg-white/20 hover:border-white/20 transition-all duration-300 group flex flex-col justify-between overflow-hidden">
                <div className="h-44 w-full bg-black/50 overflow-hidden relative">
                  <img src={`/img/${imgName}`} alt={item.name} className="w-full h-full object-cover group-hover:scale-110 group-hover:rotate-1 transition-transform duration-500 opacity-90 group-hover:opacity-100" onError={(e) => { e.target.src = 'https://via.placeholder.com/300?text=No+Image'; }} />
      ...
      {/* Mobile Floating Action Button (Only visible on small screens when cart has items and drawer is closed) */}
      ...
      {/* Right Side Column (Cart) - Modal on Mobile, Sidebar on Desktop */}
      <div className={`
        ...
        <div className="bg-black/90 lg:bg-black/40 backdrop-blur-xl rounded-3xl border border-white/10 shadow-2xl flex flex-col h-full lg:h-full overflow-hidden relative mt-auto lg:mt-0 max-h-[90vh] lg:max-h-full slide-up lg:animate-none">
          ...
          <div className="px-6 py-5 border-b border-white/10 bg-white/5 flex items-center justify-between flex-shrink-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                ...
                className="lg:hidden p-1.5 bg-white/10 hover:bg-white/20 rounded-full text-white transition-colors"
          ...
          <div className="p-6 border-b border-white/10 bg-black/40 flex flex-col gap-2 flex-shrink-0">
            ...
            <div className="relative">
              ...
              <input 
                type="number" min="1" placeholder="Enter table number..." value={tableNumber} onChange={(e) => setTableNumber(e.target.value)}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent">
            ...
            {cart.length === 0 ? (
              ...
              cart.map(item => (
                <div key={item._id} className="flex items-center justify-between group text-sm bg-white/5 border border-white/10 p-3 rounded-xl hover:bg-white/10 transition-colors">
                  ...
                  <div className="flex items-center gap-3 bg-black/50 border border-white/10 rounded-lg p-1 min-w-[90px] justify-between">
                    <button onClick={() => removeFromCart(item._id)} className="p-1.5 rounded-md text-gray-300 hover:bg-white/20 hover:text-white transition-colors active:scale-95"><Minus size={14} /></button>
          ...
          <div className="p-6 border-t border-white/10 bg-black/60 flex-shrink-0">
            ...
            <button
              onClick={submitOrder} disabled={cart.length === 0 || isSubmitting || !tableNumber}
